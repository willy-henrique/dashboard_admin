...
export async function GET() {
  try {
    ...
    // Lista todas as pastas dentro de Documentos/
    const [files] = await adminStorage.getFiles({ prefix: 'Documentos/', delimiter: '/' })
    // O Admin SDK não retorna prefixes diretamente via getFiles com delimiter em todos ambientes,
    // então agrupamos por diretório com base no caminho
    const dirMap = new Map<string, any[]>()
    for (const file of files) {
      const name = file.name // ex: Documentos/{providerId}/arquivo.png
    ...
    for (const [providerId, providerFiles] of dirMap.entries()) {
      ...
      providers.push({ providerId, documents, uploadedAt: new Date(uploadedAt || Date.now()) })
    }
